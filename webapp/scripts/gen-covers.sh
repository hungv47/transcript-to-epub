...
#
# Optional: AI_IMAGE_MODEL=<creator/model> to pick a model (else gateway default).
set -euo pipefail
cd "$(dirname "$0")/.."   # -> webapp/
...
while IFS=$'\t' read -r slug title author; do
  ...
  echo "→ generating cover: $slug"
  args=(image "$prompt" --aspect-ratio 2:3 --size 1024x1536 -o "$OUT/$slug.png" --quiet --no-preview)
  [ -n "${AI_IMAGE_MODEL:-}" ] && args+=(-m "$AI_IMAGE_MODEL")
  ai "${args[@]}"
